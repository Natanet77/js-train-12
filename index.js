...
function transformDictionary(dictionary) {
  ...
  for (const entrie of dictionary.entries()) {
    entries.push(entrie);
  }
...
function getFilteredDictionarySize(dictionary, filter) {
  const getFilter = new Map();
  for (const [key, value] of dictionary.entries()) {
    if (filter(key, value)) {
      getFilter.set(key, value);
    }
  }
...
function sortByValues(dictionary) {
  ...
  array.sort((elem1, elem2) => elem2[1] - elem1[1]);
